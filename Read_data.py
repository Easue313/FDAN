...
class ReadData1:
    ...
    def read_data_file(self):
        ...
        for key in labels.keys():
            file_data = hdf5storage.loadmat(os.path.join(r"../../Data", key))
            file_data = np.array(file_data).reshape(-1, 1)
            file_data = file_data[:self.args.data_length, :]
            data_resample = file_data
            win_num = data_resample.shape[0] // self.args.data_length
            [x.append(data_resample[idx * self.args.data_length:idx * self.args.data_length + self.args.data_length])
