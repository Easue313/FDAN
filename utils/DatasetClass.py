...
class SimpleDataset_add(Dataset):
    """
       A Dataset class supporting data, class labels, and domain labels.
       """

    def __init__(self, data_content):
        '''
        The 'data_content' should be a dict with three keys:
        ('data', 'label', 'domain_labels')
        '''
        self.data = data_content['data']
        self.class_label = data_content['label']
        self.domain_label = data_content['domain_labels']

    def __getitem__(self, index):
        data_i = self.data[index]
        class_label_i = self.class_label[index]
        domain_label_i = self.domain_label[index]
        return data_i, class_label_i, domain_label_i

    def __len__(self):
        if not (self.data.shape[0] == self.class_label.shape[0] == self.domain_label.shape[0]):
            raise ValueError('The number of samples, class labels, and domain labels should be equal!')
